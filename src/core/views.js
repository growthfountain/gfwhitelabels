...
class FormView extends View {
    constructor(template, model = null, selector = "#content", directives = [], events = {}, ...args) {
        if (model === null) {
            throw 'Please provide model for FormView';
        }
        super(template, model, selector, directives, events, ...args);
    }
    submit(e, newData = null) {
        e.preventDefault();
        ...
        if (Object.keys(errors).length !== 0) {
            ...
            this.model.save().then((response) => {
                ...
                if (defaultAction == 1) {
                    ...
                    e.target.removeAttribute('disabled');
                    $('body').scrollTo();
                    app.routers.navigate(this.getSuccessUrl(response), { trigger: true, replace: false });
                }
            });
        }
        /*
